...
interface DayData { date: string; activity: number; isPast: boolean }
...
function generateData(density = 0.6): DayData[][] {
  ...
  for (let w = 51; w >= 0; w--) {
    ...
    for (let d = 0; d < 7; d++) {
      ...
      const activity = !isPast ? 0
        : w > 40 ? (rand > (1 - density * 0.5) ? 0 : Math.ceil(rand * 3))
        : w > 20 ? (rand > (1 - density * 0.7) ? 0 : Math.ceil(rand * 4))
        : (rand > (1 - density) ? Math.ceil(rand * 4) : 0)
...
const habitTrackers: Tracker[] = [
  ...
  { id: "writing",   label: "Writing",    emoji: "✍️", color: "#db2777", currentStreak: 5,  longestStreak: 19, totalDays: 63,  data: generateData(0.50), description: "Field notes & brain dumps" },
...
const ALL_TABS: Tab[] = [
  {
    id: "overall", label: "Overall", emoji: "◎", color: "#7c3aed",
  ...
  {
    id: "code", label: "Code", emoji: "⬡", color: "#059669",
    trackers: [{ id: "code", label: "Code", emoji: "⬡", color: "#059669", currentStreak: 6, longestStreak: 31, totalDays: 142, data: generateData(0.70) }],
  },
  {
    id: "design", label: "Design", emoji: "◻", color: "#db2777",
    trackers: [{ id: "design", label: "Design", emoji: "◻", color: "#db2777", currentStreak: 5, longestStreak: 18, totalDays: 87, data: generateData(0.60) }],
  },
  ...
  { id: "tea",       label: "Tea ritual", emoji: "🍵", color: "#2563eb", trackers: [habitTrackers[2]] },
]
...
export function StreakTracker() {
  ...
  const getColor = (activity: number) =>
    activity === 0 ? "rgba(0,0,0,0.05)" : hexToAlpha(color, 0.15 + (activity / 4) * 0.75)
      ...
      <div style={{ maxWidth: 820, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 28 }}>
          {ALL_TABS.map(t => (
            <button
              key={t.id}
              onClick={() => { setActiveTab(t.id); setHovered(null) }}
              style={{
              ...
              {activeTab === t.id && (
                <span style={{ fontFamily: "monospace", fontSize: 10, background: t.color + "25", borderRadius: 999, padding: "1px 6px" }}>
        ...
        <div style={{ background: "#fff", borderRadius: 16, border: "1px solid rgba(0,0,0,0.08)", padding: "20px 24px" }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", marginTop: 8 }}>
            {months.map(m => <span key={m} style={{ fontFamily: "monospace", fontSize: 9, color: "#9ca3af" }}>{m}</span>)}
          </div>
