...
const rabbitHolePaths: RabbitHolePath[] = [
  {
    ...
    stages: ["Why taste is subjective", "Psychology of preference", "Design & perception"],
  },
  {
    id: "memory-path",
    theme: "Mind + Recall",
    category: "deep dives",
    stages: ["Why we misremember details", "Memory distortions in daily life", "Building systems for better recall"],
  },
  {
    id: "signal-path",
    theme: "Culture + Signals",
    category: "niche topics",
    stages: ["How trends mutate online", "Memetics & group identity", "Taste clusters in digital spaces"],
  },
];
    ...
    <section ref={ref} className="relative px-4 sm:px-6 pt-16 sm:pt-16 pb-8 sm:pb-12">
      <div className={cn("mx-auto max-w-7xl opacity-0", isVisible && "animate-fade-in-up")}>
        <div className="space-y-2 mb-8">
          ...
          <p className="text-base sm:text-lg text-muted-foreground leading-relaxed">Curate fascinating things I keep falling into: niche topics, random skills, deep dives, and open-ended curiosity trails</p>
        </div>
        ...
        <div className="rounded-xl border border-border bg-card/40 glass p-5 sm:p-6">
              ...
              return (
                ...
                >
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    ...
                    <span className={cn("rounded-full border px-2.5 py-1 text-[10px] font-mono tracking-[0.18em]", categoryStyleMap[path.category])}>{path.category}</span>
                  </div>
          ...
          <div className={cn("mt-5 rounded-lg border border-dashed border-primary/35 bg-primary/5 px-4 py-3 text-sm text-muted-foreground opacity-0", isVisible && "animate-fade-in-up")} style={{ animationDelay: "560ms" }}>
            <p className="font-mono text-[10px] tracking-[0.2em] text-primary flex items-center gap-1.5 mb-1.5">
              <Sparkles className="h-3.5 w-3.5" />
              rabbit-hole logic;
            </p>
            <p>One question opens another, then another. The goal is not to finish quickly. It is to follow the thread until a new angle appears.</p>
          </div>
        </div>
      </div>
    </section>
