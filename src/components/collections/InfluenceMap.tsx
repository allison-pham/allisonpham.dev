...
export function InfluenceMap() {
  ...
  const filtered = influences.filter((inf) => (domainFilter === "all" || inf.domain === domainFilter) && (typeFilter === "all" || inf.type === typeFilter));

  const grouped = filtered.reduce<Record<InfluenceDomain, typeof influences>>(
    (acc, inf) => {
    ...
    <section ref={sectionRef} className="overflow-x-clip px-4 pt-10 pb-12 sm:px-6 sm:pt-16 sm:pb-20">
      <div className="mx-auto max-w-7xl">
        <div className={cn("mb-10 space-y-4 opacity-0", isVisible && "animate-fade-in-up")}>
          <p className="font-mono text-xs tracking-[0.25em] text-primary flex items-center gap-2">intellectual lineage;</p>
          <h2 className="text-3xl font-bold tracking-tight sm:text-4xl lg:text-5xl">Influence Map ◉</h2>
          <p className="text-base sm:text-lg text-muted-foreground leading-relaxed">The people, books, papers, and projects that shaped how I think</p>
        </div>
        ...
        <div className={cn("mb-8 flex flex-wrap gap-6 opacity-0", isVisible && "animate-fade-in-up")} style={{ animationDelay: "80ms" }}>
          <div className="flex flex-wrap gap-2">
            ...
            <button
              ...
              className={cn(
                "rounded-full border px-3 py-1 font-mono text-[10px] tracking-wider transition-all",
                domainFilter === "all" ? "border-primary bg-primary/15 text-primary" : "border-border text-muted-foreground hover:border-foreground/40",
              )}
            >
              all
            </button>
            {(Object.keys(influenceDomainConfig) as InfluenceDomain[]).map((d) => (
              <button
                ...
                style={{
                  borderColor: domainFilter === d ? influenceDomainConfig[d].color : undefined,
                  background: domainFilter === d ? influenceDomainConfig[d].bg : undefined,
                  color: domainFilter === d ? influenceDomainConfig[d].color : undefined,
                }}
              >
                {d}
              </button>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {types.map((t) => (
              ...
              >
                {t !== "all" && <span className="mr-1">{influenceTypeIcon[t]}</span>}
                {t}
              </button>
            ))}
          </div>
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2 space-y-6">
            {domains.map((domain, di) => {
              ...
              return (
                ...
                >
                  <p className="mb-4 font-mono text-[10px] tracking-[0.2em]" style={{ color: cfg.color }}>
                    {domain}
                  </p>
                  <div className="flex flex-wrap gap-2">
                    {grouped[domain].map((inf) => (
                      <button
                        ...
                        className={cn(
                          ...
                          selected?.id === inf.id ? "bg-card/80 shadow-sm" : "border-border/50 bg-card/20 hover:border-opacity-50",
                        )}
                        style={selected?.id === inf.id ? { borderColor: cfg.color, background: cfg.bg } : undefined}
                      >
                        <span className="text-[11px]" style={{ color: cfg.color }}>
                          {influenceTypeIcon[inf.type]}
                        </span>
                        <span
                          className={cn("font-mono text-[11px] font-medium tracking-wide transition-colors", selected?.id === inf.id ? "" : "text-muted-foreground group-hover:text-foreground")}
                          style={selected?.id === inf.id ? { color: cfg.color } : undefined}
                        >
                          {inf.name}
                        </span>
                        {inf.year && <span className="font-mono text-[9px] text-muted-foreground">{inf.year}</span>}
                      </button>
                    ))}
                  </div>
                </div>
          ...
          <div className="space-y-4">
            {selected ? (
              <div className="rounded-xl border bg-card/40 glass p-6 space-y-4 animate-fade-in-up" style={{ borderColor: influenceDomainConfig[selected.domain].border }}>
                <div className="flex items-start justify-between gap-2">
                  ...
                  <button onClick={() => setSelected(null)} className="font-mono text-xs text-muted-foreground hover:text-primary shrink-0">
                    ×
                  </button>
                </div>
                ...
                {selected.url && (
                  <a href={selected.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 font-mono text-xs text-muted-foreground transition-colors hover:text-primary">
