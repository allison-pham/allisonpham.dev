...
function FlavorOverlay() {
  ...
  return (
    <svg width="100%" viewBox={`0 0 ${size} ${size}`} className="mx-auto max-w-xs">
      {gridLevels.map((lv) => (
        <polygon key={lv} points={axisPoints.map(({ x, y }) => `${cx + (x - cx) * lv},${cy + (y - cy) * lv}`).join(" ")} fill="none" stroke="rgba(167,139,250,0.12)" strokeWidth="0.8" />
      ))}
      {axisPoints.map((p, i) => (
      ...
      {teaPolygons.map(({ entry, pts }) => {
        ...
        return <polygon key={entry.id} points={pts} fill={cfg.bg} stroke={cfg.color} strokeWidth="1.2" opacity={0.6} />;
      })}
      {keys.map((k, i) => {
        const a = (i / n) * Math.PI * 2 - Math.PI / 2;
        ...
        return (
          <text key={k} x={lx} y={ly} textAnchor="middle" dominantBaseline="middle" fontSize="8.5" fill="rgba(160,160,185,0.85)" fontFamily="'Geist Mono',monospace">
            {k}
          </text>
        );
      })}
    ...
    <div className="space-y-3">
      ...
      {teaLog.map((entry) => (
        <div key={entry.id} className="flex items-center gap-3">
          ...
          <div className={cn("flex-1 rounded-lg border px-3 py-2 text-center", entry.mood !== entry.moodAfter ? "border-primary/30 bg-primary/5" : "border-border/40 bg-secondary/30")}>
  ...
  return (
    <div className="overflow-x-auto">
      <svg width="100%" viewBox={`0 0 ${W} ${H}`} className="rounded-lg border border-border/30 bg-secondary/20">
        ...
        {teaLog.map((entry) => {
          ...
          return (
            <g key={entry.id}>
              ...
              <text x={x} y={y - 11} textAnchor="middle" fontSize="6.5" fill="rgba(200,200,220,0.8)" fontFamily="'Geist Mono',monospace">
                {entry.name.split(" ")[0]}
              </text>
            </g>
          );
        ...
        {/* Axis labels */}
        <text x={4} y={H / 2} fontSize="7" fill="rgba(120,120,140,0.6)" fontFamily="'Geist Mono',monospace">
          0°
        </text>
        <text x={W / 2 - 8} y={H - 3} fontSize="7" fill="rgba(120,120,140,0.6)" fontFamily="'Geist Mono',monospace">
          180°
        </text>
      </svg>
    </div>
    ...
    <section ref={sectionRef} className="overflow-x-clip px-4 pt-10 pb-12 sm:px-6 sm:pt-16 sm:pb-20">
      <div className="mx-auto max-w-3xl">
        <div className={cn("mb-10 space-y-4 opacity-0", isVisible && "animate-fade-in-up")}>
          <p className="font-mono text-xs tracking-[0.25em] text-primary flex items-center gap-2">tasting data;</p>
          <h2 className="text-3xl font-bold tracking-tight sm:text-4xl lg:text-5xl">Tea Tasting Viz 📊</h2>
          <p className="text-base sm:text-lg text-muted-foreground leading-relaxed">The tea log (visualized) - flavor overlays, focus ratings, mood shifts, and origins</p>
        </div>
        ...
        <div className={cn("mb-8 grid grid-cols-2 gap-3 sm:grid-cols-4 opacity-0", isVisible && "animate-fade-in-up")} style={{ animationDelay: "80ms" }}>
          {MODES.map((m) => (
            <button
              ...
              className={cn("rounded-xl border p-3 text-left transition-all duration-300 active:scale-[0.98] hover-lift", mode === m.value ? "border-primary/40 bg-primary/10" : "border-border/50 bg-card/30 glass hover:border-primary/20")}
            >
              <p className={cn("font-mono text-[10px] font-semibold tracking-wider", mode === m.value ? "text-primary" : "text-muted-foreground")}>{m.label}</p>
            </button>
          ))}
        </div>
