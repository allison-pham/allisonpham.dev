      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className={cn("mb-12 space-y-4 opacity-0", isVisible && "animate-fade-in-up")}>
          ...
          <p className="max-w-xl text-base leading-relaxed text-muted-foreground">
            Interfaces I've used, broken, been delighted by, or confused by - annotated with the
        ...
        {/* Type counts */}
        <div
          className={cn("mb-8 grid grid-cols-2 gap-3 sm:grid-cols-4 opacity-0", isVisible && "animate-fade-in-up")}
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  typeFilter === f.value
                    ? cfg.pill
                    : "border-border/50 bg-card/40 glass hover:border-primary/20"
                )}
              >
        ...
        {/* Tag filter */}
        <div
          className={cn("mb-8 flex flex-wrap items-center gap-2 opacity-0", isVisible && "animate-fade-in-up")}
              ...
              >
                ...
                >
                  ...
                  <span className={cn("mt-1.5 h-2 w-2 shrink-0 rounded-full", cfg.dot,
                    note.type === "confusion" && "animate-pulse")} />

                  <div className="flex-1 min-w-0 space-y-1.5">
                    {/* Observation preview */}
                    <p className={cn(
                      "text-sm leading-relaxed text-foreground transition-colors",
                      isOpen ? "text-foreground" : "line-clamp-2 text-muted-foreground group-hover:text-foreground"
                    )}>
                      {note.observation}
                    </p>
                  ...
                  <div className="flex shrink-0 items-center gap-2">
                    <span className={cn(
                      "hidden sm:inline-flex rounded-full border px-2 py-0.5 font-mono text-[9px] tracking-wider",
                      cfg.pill
                    )}>
                      {cfg.label}
                    </span>
                ...
                {isOpen && (
                  <div className="border-t border-border/40 px-6 pb-5 pt-4 space-y-4 animate-fade-in-up">
                    <div className="rounded-lg border border-primary/20 bg-primary/5 px-4 py-3">
                      <p className="mb-1 font-mono text-[10px] tracking-widest text-primary">
                        principle
                      </p>
        ...
        {/* Footer note */}
        <div
          className={cn("mt-8 rounded-xl border border-primary/20 bg-primary/5 px-6 py-5 opacity-0", isVisible && "animate-fade-in-up")}
          style={{ animationDelay: "600ms" }}
        >
          <p className="text-sm leading-relaxed text-muted-foreground">
            These are real observations from daily life - not curated examples from a textbook.
            The point is to notice the interfaces that shape how people think, even the ones nobody talks about.
          </p>
        </div>
