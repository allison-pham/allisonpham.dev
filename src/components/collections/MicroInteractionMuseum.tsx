...
function HoldToConfirm() {
  ...
  return (
    ...
    >
      ...
      <span className={cn("relative font-mono text-xs font-medium transition-colors", done ? "text-red-400" : "text-red-400/80")}>{done ? "✓ confirmed" : progress > 0 ? "hold…" : "hold to delete"}</span>
    </button>
  );
}
...
function SpringButton() {
  ...
  return (
    <button
      ...
      className={cn("relative h-12 w-36 rounded-xl border border-primary/40 bg-primary/10 font-mono text-xs text-primary select-none cursor-pointer", "transition-transform duration-100", pressed ? "scale-90" : "scale-100 hover:scale-105")}
...
function CounterTick() {
  ...
  return (
    <div className="flex items-center gap-3">
      <button onClick={() => change("down")} className="flex h-9 w-9 items-center justify-center rounded-lg border border-border/50 bg-secondary/40 font-mono text-sm text-foreground hover:border-primary/40 hover:text-primary transition-colors">
        −
      </button>
      <div className="w-12 overflow-hidden text-center">
        <span className={cn("block font-mono text-2xl font-bold text-foreground transition-all duration-120", animating && dir === "up" && "-translate-y-3 opacity-0", animating && dir === "down" && "translate-y-3 opacity-0")}>{count}</span>
      </div>
      <button onClick={() => change("up")} className="flex h-9 w-9 items-center justify-center rounded-lg border border-border/50 bg-secondary/40 font-mono text-sm text-foreground hover:border-primary/40 hover:text-primary transition-colors">
        +
      </button>
    </div>
  );
...
function ToggleDemo() {
  ...
  return (
    <button
      onClick={() => setOn((v) => !v)}
      className={cn("relative flex h-7 w-14 items-center rounded-full border-2 transition-all duration-300 cursor-pointer", on ? "border-primary bg-primary/20" : "border-border bg-secondary/40")}
      style={{ padding: "2px" }}
    >
      <span className={cn("h-5 w-5 rounded-full shadow-sm transition-all", on ? "bg-primary translate-x-7" : "bg-muted-foreground translate-x-0")} style={{ transition: "all 0.35s cubic-bezier(0.34, 1.56, 0.64, 1)" }} />
    </button>
  );
}
    ...
    >
      {done ? (
        <svg width="16" height="16" viewBox="0 0 16 16">
          <path d="M3 8 L6.5 11.5 L13 5" fill="none" stroke="#4ade80" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" strokeDasharray="20" strokeDashoffset={done ? 0 : 20} style={{ transition: "stroke-dashoffset 0.4s ease" }} />
        </svg>
      ) : (
    ...
    <div className="flex flex-col gap-2">
      <div className={cn("flex gap-2", shaking && "animate-[shake_0.5s_ease]")}>
        <input
          ...
          className={cn(
            "w-32 rounded-lg border bg-secondary/30 px-3 py-2 font-mono text-xs outline-none transition-colors",
            shaking ? "border-red-500/60 text-red-400" : success ? "border-green-500/50 text-green-400" : "border-border/50 text-foreground",
            "focus:border-primary",
          )}
        />
        <button onClick={submit} className="rounded-lg border border-border/50 bg-secondary/40 px-3 py-2 font-mono text-xs text-muted-foreground hover:border-primary/40 hover:text-primary transition-colors">
          go
        </button>
      </div>
      <p className="font-mono text-[9px] text-muted-foreground">{shaking ? "⚡ too short!" : success ? "✓ ok!" : "enter ≥3 chars"}</p>
    ...
    <section ref={sectionRef} className="overflow-x-clip px-4 pt-10 pb-12 sm:px-6 sm:pt-16 sm:pb-20">
      <div className="mx-auto max-w-7xl">
        <div className={cn("mb-10 space-y-4 opacity-0", isVisible && "animate-fade-in-up")}>
          <p className="font-mono text-xs tracking-[0.25em] text-primary flex items-center gap-2">interaction design;</p>
          <h2 className="text-3xl font-bold tracking-tight sm:text-4xl lg:text-5xl">Micro-Interaction Museum ✦</h2>
          <p className="text-base sm:text-lg text-muted-foreground leading-relaxed">Live demos of micro-interactions worth studying. Each one is interactive - play with it, then read why it works.</p>
        </div>
        ...
        <div className={cn("mb-8 flex flex-wrap gap-2 opacity-0", isVisible && "animate-fade-in-up")} style={{ animationDelay: "80ms" }}>
          {CATEGORIES.map((c) => (
            <button
              ...
              style={c !== "all" && catFilter === c ? { borderColor: categoryConfig[c].color, background: categoryConfig[c].bg, color: categoryConfig[c].color } : catFilter === c ? undefined : {}}
            >
              <span className={cn(catFilter === c && c === "all" ? "text-primary" : catFilter !== c ? "text-muted-foreground" : "")}>{c}</span>
            </button>
          ))}
        </div>
        ...
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((exhibit, i) => {
            ...
            return (
              <div
                key={exhibit.id}
                className={cn("group relative overflow-hidden rounded-xl border bg-card/40 glass transition-all duration-300 opacity-0", isVisible && "animate-fade-in-up", isOpen ? "border-primary/30" : "border-border/50 hover:border-primary/20")}
                style={{ animationDelay: `${i * 50 + 150}ms` }}
              >
                {/* Category badge */}
                <div className="flex items-center justify-between px-5 pt-4 pb-2">
                  <span className="rounded-md border px-2 py-0.5 font-mono text-[9px] tracking-wider" style={{ color: cfg.color, borderColor: cfg.border, background: cfg.bg }}>
                    {exhibit.category}
                  </span>
                ...
                {/* Title + expand */}
                <button onClick={() => setExpanded(isOpen ? null : exhibit.id)} className="flex w-full items-center justify-between gap-3 border-t border-border/30 px-5 py-3 text-left">
                  <p className={cn("font-mono text-xs font-semibold tracking-wider transition-colors", isOpen ? "text-primary" : "text-foreground group-hover:text-primary")}>{exhibit.title}</p>
                  <span className="font-mono text-[10px] text-muted-foreground shrink-0">{isOpen ? "hide ↑" : "why ↓"}</span>
                </button>
                ...
                {isOpen && (
                  <div className="border-t border-border/30 px-5 pb-5 pt-3 animate-fade-in-up space-y-2">
                    ...
                    {exhibit.source && <p className="font-mono text-[9px] text-muted-foreground">seen in: {exhibit.source}</p>}
                  </div>
                )}

                <div className="absolute bottom-0 left-0 h-0.5 w-0 rounded-full bg-linear-to-r from-primary via-primary/80 to-transparent transition-all duration-500 group-hover:w-full" />
              </div>
            );
          })}
        </div>
