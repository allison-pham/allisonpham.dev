...
export function Bookshelf() {
  ...
  const filtered = books.filter(
    (b) => (statusFilter === "all" || b.status === statusFilter) && (!tagFilter || b.tags.includes(tagFilter)),
  );
    ...
    <section ref={sectionRef} className="overflow-x-clip px-4 pt-10 pb-12 sm:px-6 sm:pt-16 sm:pb-20">
      <div className="mx-auto max-w-7xl">
        <div className={cn("mb-10 space-y-4 opacity-0", isVisible && "animate-fade-in-up")}>
          <p className="font-mono text-xs tracking-[0.25em] text-primary flex items-center gap-2">reading shelf;</p>
          <h2 className="text-3xl font-bold tracking-tight sm:text-4xl lg:text-5xl">Bookshelf</h2>
          <p className="text-base sm:text-lg text-muted-foreground leading-relaxed">Books that changed how I think</p>
        </div>

        <div
          className={cn("mb-8 flex flex-wrap gap-4 opacity-0", isVisible && "animate-fade-in-up")}
          ...
          <div className="flex flex-wrap gap-2">
            {STATUS_FILTERS.map((s) => (
              <button
                ...
                className={cn(
                  "flex items-center gap-2 rounded-full border px-3 py-1 font-mono text-[10px] tracking-wider transition-all active:scale-[0.98]",
                  statusFilter === s
              ...
              <div className="rounded-xl border border-primary/30 bg-card/40 glass p-6 space-y-4 animate-fade-in-up sticky top-24">
                <div className="flex gap-4">
                  ...
                  <div className="flex-1 min-w-0 space-y-1">
                    ...
                    {selected.year && (
                      <p className="font-mono text-[10px] text-muted-foreground">read {selected.year}</p>
                    )}
                  ...
                  {selected.tags.map((tag) => (
                    <button
                      ...
                      className={cn(
                        ...
                        tagFilter === tag
                          ? "border-primary/50 bg-primary/10 text-primary"
                          : "border-border/60 bg-secondary/40 text-muted-foreground hover:text-primary",
                      )}
                    >
                      {tag}
                    </button>
                  ...
                  {STATUS_FILTERS.slice(1).map((s) => (
                    <div key={s} className="flex items-center gap-2">
                      ...
                      <span className="font-mono text-[10px] text-muted-foreground">
                        {statusConfig[s as ReadingStatus].label}
                      </span>
                    </div>
                  ))}
                </div>
