...
function EntryCard({ entry, compact = false }: { entry: CalendarEntry; compact?: boolean }) {
  ...
  return (
    <div className={cn(
      ...
      <div className={cn("absolute inset-0 bg-gradient-to-br opacity-40", cfg.bg)} />
  ...
  return (
    <div className="rounded-xl border border-border/50 bg-card/40 glass p-5">
      ...
      <div className="mb-2 grid grid-cols-7 gap-1">
        {DAYS.map((d) => (
          <div key={d} className="text-center font-mono text-[9px] tracking-wider text-muted-foreground py-1">{d}</div>
        ))}
      </div>
  ...
  return (
    <div className="rounded-xl border border-border/50 bg-card/40 glass p-5 space-y-4">
      ...
      <div className="relative space-y-0">
        {/* Timeline line */}
        <div className="absolute left-[52px] top-0 bottom-0 w-px bg-border/40" />
        {dailies.map((entry, i) => (
    ...
    <section ref={sectionRef} className="overflow-x-clip px-4 pt-10 pb-12 sm:px-6 sm:pt-16 sm:pb-20">
      <div className="mx-auto max-w-7xl">
        ...
        <div className={cn("mb-12 space-y-4 opacity-0", isVisible && "animate-fade-in-up")}>
          <p className="font-mono text-xs tracking-[0.25em] sm:tracking-[0.35em] text-primary">in the world;</p>
          <h2 className="text-2xl font-bold tracking-tight sm:text-4xl lg:text-5xl">Calendar ◈</h2>
          <p className="text-base sm:text-lg text-muted-foreground leading-relaxed">Events I've been at, what I'm building, and what a typical day looks like (slice of how I spend my time)</p>
        </div>
          ...
          {TYPE_FILTERS.map((t) => (
            <button key={t} onClick={() => setTypeFilter(t)}
              className={cn(
                ...
                typeFilter === t
                  ? t === "all" ? "border-primary bg-primary/15 text-primary" : typeConfig[t as EntryType].pill
            ...
            {selectedDate && (
              <div className="rounded-xl border border-primary/30 bg-card/40 glass p-5 space-y-4 animate-fade-in-up">
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => setSelectedDate("")} className="font-mono text-xs text-muted-foreground hover:text-primary transition-colors">×</button>
                </div>
                {dateEntries.length > 0 ? (
                  <div className="space-y-3">
                    {dateEntries.map((e) => <EntryCard key={e.id} entry={e} compact />)}
                  </div>
                ) : (
