...
function FlavorRadar({ flavors }: { flavors: Record<string, number> }) {
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className="overflow-visible">
      ...
      {/* Data polygon */}
      <polygon points={dataPoints.map((p) => `${p.x},${p.y}`).join(" ")} fill="rgba(167,139,250,0.2)" stroke="rgba(167,139,250,0.7)" strokeWidth="1.5" />
      {/* Labels */}
      {keys.map((k, i) => {
        const angle = (i / n) * Math.PI * 2 - Math.PI / 2;
        ...
        return (
          <text key={k} x={lx} y={ly} textAnchor="middle" dominantBaseline="middle" fontSize="7" fill="rgba(160,160,180,0.8)" fontFamily="'Geist Mono', monospace">
            {k}
          </text>
        );
      })}
...
export function TeaLog() {
  ...
  const filtered = teaLog.filter((t) => typeFilter === "all" || t.type === typeFilter).sort((a, b) => b.date.localeCompare(a.date));
    ...
    <section ref={sectionRef} className="overflow-x-clip px-4 pt-10 pb-12 sm:px-6 sm:pt-16 sm:pb-20">
      <div className="mx-auto max-w-7xl">
        <div className={cn("mb-10 space-y-4 opacity-0", isVisible && "animate-fade-in-up")}>
          <p className="font-mono text-xs tracking-[0.25em] text-primary flex items-center gap-2">tasting log;</p>
          <h2 className="text-3xl font-bold tracking-tight sm:text-4xl lg:text-5xl">Tea Log 🍵</h2>
          <p className="text-base sm:text-lg text-muted-foreground leading-relaxed">Every tea I've brewed worth logging - origin, flavor profile, and what it did to my focus</p>
        </div>
        ...
        <div className={cn("mb-8 flex flex-wrap gap-2 opacity-0", isVisible && "animate-fade-in-up")} style={{ animationDelay: "80ms" }}>
          {TYPE_FILTERS.map((t) => (
            ...
            >
              <span className={cn(typeFilter !== t && "text-muted-foreground")}>{t === "all" ? "all" : teaTypeConfig[t].label.toLowerCase()}</span>
            </button>
          ))}
        </div>
        ...
        <div className="flex flex-col gap-3">
          {filtered.map((entry, i) => {
            ...
            return (
              ...
              >
                <button onClick={() => setExpanded(isOpen ? null : entry.id)} className="flex w-full items-start gap-4 px-5 py-4 text-left sm:px-6">
                  {/* Type badge */}
                  <span className="mt-0.5 shrink-0 rounded-md border px-2 py-0.5 font-mono text-[9px] tracking-wider" style={{ color: cfg.color, borderColor: cfg.border, background: cfg.bg }}>
                    {cfg.label.toLowerCase()}
                  </span>
                  <div className="flex-1 min-w-0 space-y-1">
                    <p className={cn("font-mono text-sm font-semibold tracking-wider transition-colors", isOpen ? "text-primary" : "text-foreground group-hover:text-primary")}>{entry.name}</p>
                    <p className="font-mono text-[10px] text-muted-foreground">
                      {entry.origin} · {formatDate(entry.date)}
                    </p>
                  </div>
                  <div className="flex shrink-0 items-center gap-3">
                    <span className="font-mono text-xs">
                      {moodEmoji[entry.mood]} → {moodEmoji[entry.moodAfter]}
                    </span>
                  ...
                  <div className="border-t border-border/40 px-5 pb-5 pt-4 sm:px-6 animate-fade-in-up">
                    <div className="grid gap-6 sm:grid-cols-2">
                      ...
                      <div className="space-y-4">
                        ...
                        <p className="text-sm leading-relaxed text-muted-foreground border-l-2 border-primary/30 pl-3 italic">{entry.notes}</p>
                      </div>
                      {/* Right: radar */}
