"use client";
import { Camera, Calendar, MapPin, X, ChevronLeft, ChevronRight, Heart, MessageCircle, Share2 } from "lucide-react";
    ...
    <section ref={ref} className="relative px-4 sm:px-6 pt-16 sm:pt-16 pb-8 sm:pb-12">
      <div className={cn("mx-auto w-full max-w-7xl opacity-0", isVisible && "animate-fade-in-up")}>
        <div className="space-y-2 mb-8">
          ...
          <p className="text-base sm:text-lg text-muted-foreground leading-relaxed">Moments from life captured through photos</p>
        </div>
        ...
        <div className="grid gap-6 sm:grid-cols-2">
          {photoDumps.map((dump, i) => (
            <button
              ...
              className={cn("group text-left rounded-xl border border-border bg-card/40 glass overflow-hidden opacity-0 hover:border-primary/50 transition-all", isVisible && "animate-fade-in-up")}
              style={{ animationDelay: `${i * 100}ms` }}
            >
              {/* Preview grid */}
              <div className="grid grid-cols-3 gap-0.5 p-1">
                {dump.photos.slice(0, 3).map((photo, idx) => (
                  <div key={photo.id} className={cn("aspect-square bg-secondary/50 rounded overflow-hidden", idx === 0 && "col-span-2 row-span-2 aspect-auto")}>
              ...
              <div className="p-4">
                <div className="flex items-center gap-2 flex-wrap mb-2">
                  ...
                  {dump.mood && <span className={cn("px-2 py-0.5 rounded-full border font-mono text-[9px]", moodColors[dump.mood] || "bg-secondary text-muted-foreground border-border")}>{dump.mood}</span>}
                </div>
      ...
      {/* Modal */}
      {selectedDump && (
        <div className="fixed inset-0 z-50 bg-background/50 backdrop-blur-sm overflow-y-auto" onClick={() => setSelectedDump(null)}>
          <div className="max-w-4xl mx-auto px-4 py-8" onClick={(e) => e.stopPropagation()}>
            <button onClick={() => setSelectedDump(null)} className="fixed top-4 right-4 p-2 rounded-full bg-secondary hover:bg-secondary/80 transition-colors z-10">
              <X className="h-5 w-5" />
            </button>
      ...
      {lightboxIndex !== null && selectedDump && (
        <div className="fixed inset-0 z-[60] bg-black/50 backdrop-blur-sm flex items-center justify-center" onClick={() => setLightboxIndex(null)}>
          <button onClick={() => setLightboxIndex(null)} className="absolute top-4 right-4 p-2 rounded-full bg-white/10 hover:bg-white/20 transition-colors text-white">
            <X className="h-5 w-5" />
          </button>
