...
const MOODS: Record<Mood, { emoji: string; label: string; message: string; bg: string; dot: string }> = {
  focused:  { emoji: "🌿", label: "focused",  message: "in the zone. do not disturb (please).", bg: "bg-green-500/10 border-green-500/25",  dot: "bg-green-500" },
  sleepy:   { emoji: "🌙", label: "sleepy",   message: "five more minutes…",                     bg: "bg-blue-400/10 border-blue-400/25",   dot: "bg-blue-400 animate-pulse" },
  happy:    { emoji: "🌸", label: "happy",    message: "everything is soft and good today.",      bg: "bg-pink-400/10 border-pink-400/25",   dot: "bg-pink-400" },
  curious:  { emoji: "🔭", label: "curious",  message: "what if we looked at one more thing…",   bg: "bg-primary/10 border-primary/25",     dot: "bg-primary" },
  hungry:   { emoji: "🍵", label: "hungry",   message: "tea break. non-negotiable.",              bg: "bg-amber-400/10 border-amber-400/25", dot: "bg-amber-400" },
}
...
function StudyBuddy() {
  ...
  return (
    <div className={cn("flex flex-col items-center gap-4 rounded-xl border p-6 transition-all duration-500", cfg.bg)}>
      <button onClick={handleClick} aria-label="Change study buddy mood"
        className={cn("relative flex h-20 w-20 cursor-pointer flex-col items-center justify-center rounded-full border-2 border-current/20 bg-background/60 transition-transform duration-200 hover:scale-105 active:scale-95 select-none",
        ...
        <div className="flex flex-col items-center gap-0.5">
          ...
          <div className={cn("mt-1 rounded-full border-b-2 border-foreground",
      ...
      <div className="flex gap-1.5 flex-wrap justify-center">
        {MOOD_ORDER.map((m) => (
          <button key={m} onClick={() => setMood(m)}
            className={cn("rounded-full border px-2.5 py-0.5 font-mono text-[9px] tracking-wider transition-all duration-200",
...
function CharacterCard() {
  ...
  return (
    <div className={cn("group relative overflow-hidden rounded-xl border transition-all duration-500 cursor-default",
      hovered ? "border-primary/40" : "border-border/50",
      "bg-gradient-to-br from-card/80 via-card/60 to-primary/5")}
      onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
      <div className="pointer-events-none absolute inset-0 opacity-30">
        {["🌿","🌸","✦","🍄","🌙","🌾","🦋","🌻"].map((s, i) => (
          <span key={i} className="absolute text-xs transition-all duration-700"
            style={{ left: `${(i*127)%90+5}%`, top: `${(i*83)%80+5}%`,
              opacity: hovered ? 0.5 : 0.2, transform: hovered ? "scale(1.2)" : "scale(1)",
              transitionDelay: `${i*30}ms` }}>{s}</span>
        ))}
      </div>
      <div className="relative p-6 space-y-5">
        ...
        <p className={cn("font-mono text-[10px] text-primary/60 italic transition-opacity duration-300", hovered ? "opacity-100" : "opacity-0")}>
      ...
      <div className="space-y-2">
        {COLLECTIONS.map(({ name, type, emoji, note }) => (
          <div key={name} className="group flex items-center gap-3 rounded-lg border border-border/40 bg-secondary/20 px-3 py-2 hover:border-primary/30 transition-colors">
            ...
            <span className={cn("shrink-0 rounded-full border px-2 py-0.5 font-mono text-[9px]",
              type === "physical" ? "border-amber-500/30 bg-amber-500/10 text-amber-500" : "border-primary/30 bg-primary/10 text-primary")}>
              {type}
            </span>
          </div>
        ))}
      ...
      <div className="grid grid-cols-3 gap-2">
        {MOOD_TILES.map(({ label, gradient, emoji }) => (
          <div key={label} className={cn(
            "group relative flex aspect-square items-end overflow-hidden rounded-xl bg-gradient-to-br p-2 transition-transform duration-300 hover:scale-[1.04] cursor-default", gradient)}>
    ...
    <div className="rounded-xl border border-border/50 bg-card/40 glass p-5 space-y-4">
      ...
      <div className="flex flex-wrap gap-2">
        {AESTHETIC_TAGS.map(({ tag }) => (
          <button key={tag} onClick={() => setActive(active === tag ? null : tag)}
            className={cn("rounded-full border px-3 py-1.5 font-mono text-[10px] tracking-wider transition-all duration-200 active:scale-[0.98]",
              active === tag ? "border-primary bg-primary/15 text-primary shadow-sm shadow-primary/20" : "border-border/60 bg-secondary/30 text-muted-foreground hover:border-primary/30 hover:text-foreground")}>
            {tag}
          </button>
        ))}
      </div>
  ...
  return (
    <a href="/quiz" className="group relative flex items-center justify-between gap-4 overflow-hidden rounded-xl border border-primary/25 bg-primary/5 px-5 py-4 transition-all duration-300 hover:border-primary/50 hover:bg-primary/10">
      <div className="pointer-events-none absolute right-4 top-0 bottom-0 flex items-center gap-2 opacity-20 group-hover:opacity-40 transition-opacity duration-300">
        {["🌿","🌸","✦","🌙"].map((e, i) => (
          <span key={i} className="text-sm" style={{ transform: `rotate(${i*15-20}deg)` }}>{e}</span>
        ))}
      </div>
    ...
    <section ref={sectionRef} className="border-t border-border/30 px-4 pt-16 sm:pt-20 pb-8 sm:pb-12 sm:px-6">
      <div className="mx-auto max-w-7xl">
        ...
        <div className={cn("mb-10 sm:mb-14 space-y-3 opacity-0", isVisible && "animate-fade-in-up")}>
          <p className="font-mono text-xs tracking-[0.25em] sm:tracking-[0.35em] text-primary">soft things;</p>
          <h2 className="text-3xl font-bold tracking-tight sm:text-4xl lg:text-5xl">A Few Favorites ✿</h2>
          <p className="max-w-xl text-base leading-relaxed text-muted-foreground">The small things that say something true.</p>
        </div>

        {/* Row 1 - favorites + quiz · character card · study buddy */}
        <div className={cn("mb-6 grid gap-6 lg:grid-cols-3 opacity-0", isVisible && "animate-fade-in-up")} style={{ animationDelay: "80ms" }}>
        ...
        <div className={cn("mb-6 grid gap-6 lg:grid-cols-2 opacity-0", isVisible && "animate-fade-in-up")} style={{ animationDelay: "160ms" }}>
          <IfIWere />
          <MoodBoard />
        </div>

        {/* Row 3 - aesthetic tags · scents · sounds */}
        <div className={cn("mb-6 grid gap-6 lg:grid-cols-3 opacity-0", isVisible && "animate-fade-in-up")} style={{ animationDelay: "220ms" }}>
          <AestheticTags />
          <Scents />
          <SoundsCalming />
        </div>
        ...
        <div className={cn("mb-6 grid gap-6 lg:grid-cols-2 opacity-0", isVisible && "animate-fade-in-up")} style={{ animationDelay: "280ms" }}>
          <ThingsICollect />
          <WantToLearn />
        </div>

        {/* Row 5 - comfort media · places */}
        <div className={cn("grid gap-6 lg:grid-cols-2 opacity-0", isVisible && "animate-fade-in-up")} style={{ animationDelay: "340ms" }}>
          <ComfortMedia />
          <PlacesIWantToGo />
        </div>
