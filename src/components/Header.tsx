  ...
  return (
    <header
      className={cn(
        ...
        isMobileMenuOpen ? "border-b border-border/50 bg-background shadow-sm" : isScrolled ? "border-b border-border/50 bg-background/80 backdrop-blur-xl shadow-sm" : "bg-transparent",
      )}
    >
      <div className="mx-auto max-w-7xl px-4 sm:px-6 py-4">
        <nav className="flex items-center justify-between">
          <Link href="/" className="group flex items-center gap-3">
            <span className="relative inline-flex h-8 w-8 items-center justify-center overflow-hidden rounded-lg border border-primary/35 bg-card/80 transition-transform duration-300 group-hover:scale-105">
              <Image src="/website-logo.svg" alt="Allison Pham logo" fill sizes="32px" className="object-contain p-1" priority />
            </span>
            <span className="bg-linear-to-l from-primary/50 to-accent bg-clip-text text-transparent font-semibold">Allison Pham</span>
          </Link>
          ...
          <div className="hidden items-center gap-1 md:flex">
            {navItems.map((item, index) => {
              ...
              return (
                ...
                >
                  <span className={cn("absolute left-1.5 text-primary transition-all duration-200 opacity-0 -translate-x-2", (hoveredIndex === index || isActive) && "opacity-100 translate-x-0")}>{">"}</span>
                  <span className={cn("transition-transform duration-200", (hoveredIndex === index || isActive) && "translate-x-2")}>{item.label}</span>
                  <span className={cn("absolute bottom-1 left-1/2 -translate-x-1/2 h-0.5 bg-primary rounded-full transition-all duration-300", hoveredIndex === index || isActive ? "w-6" : "w-0")} />
                </Link>
              );
            })}
            ...
            <div className="relative" onMouseEnter={handleMoreEnter} onMouseLeave={handleMoreLeave}>
              <button
                className={cn("relative flex items-center gap-1 px-4 py-2.5 font-mono text-xs tracking-widest transition-all duration-300 rounded-lg hover:bg-secondary/50", isMoreOpen ? "text-primary" : "text-muted-foreground hover:text-primary")}
              >
                <span>more</span>
                ...
                {moreNavItems.map((item) => {
                  ...
                  return (
                    <Link
                      key={item.label}
                      href={item.href}
                      className={cn("flex items-center gap-2 px-4 py-3 font-mono text-xs tracking-widest transition-all duration-200 hover:bg-secondary/50", isActive ? "text-primary bg-primary/10" : "text-muted-foreground hover:text-foreground")}
                    >
          ...
          <div className="flex items-center gap-4">
            ...
            <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="flex h-10 w-10 items-center justify-center rounded-lg border border-border bg-card/50 md:hidden transition-colors hover:bg-secondary" aria-label="Toggle menu">
        ...
        <div
          className={cn(
            ...
            isMobileMenuOpen ? "max-h-[65vh] overflow-y-auto overscroll-contain opacity-100 pt-4" : "max-h-0 overflow-hidden opacity-0",
          )}
        >
